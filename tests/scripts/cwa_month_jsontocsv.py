...
# === 設定區 ===
base_path = r"F:/BackUp/CODis/cwa_yearly_data/現有站"
output_base = r"F:/BackUp/CODis/json轉csv/month"
       ...
       # print(f"處理 {station_id} - {year}")
        ...
        for file in os.listdir(station_path):
         #   if file.endswith(".json") and file.startswith(year):
            if file.endswith(".json") and year in file:
        ...
        # === 沒資料就跳過 ===
        if not all_df:
            print(f"⚠️ {station_id} {year} 沒有資料")
            continue
        ...
        # === 排序 ===
        df_year['觀測月份'] = pd.to_datetime(df_year['觀測月份'])
        df_year = df_year.sort_values(by='觀測月份')
