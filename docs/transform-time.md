...
| 型別 | 單位 | 意義 |
|------|------|------|
| `pd.DataFrame` | 不適用 | 含 `datetime.date`（或 `None`）欄的新資料表；不修改原表 |
...
| 型別 | 單位 | 意義 |
|------|------|------|
| `pd.DataFrame` | 不適用 | 含 `datetime.time`（或 `None`）欄的新資料表；不修改原表 |
...
| 型別 | 單位 | 意義 |
|------|------|------|
| `pd.DataFrame` | Timestamp（納秒解析度） | 含日期時間欄的新資料表；不修改原表 |
